import { ArrowRight, Check, Sparkles, WifiOff } from 'lucide-react'
...
export function UnderstandPanel({ pack, onBack, onNext }: UnderstandPanelProps) {
  const { t } = useI18n()
  const { text, isGenerated } = useUnderstandContent(pack)

  const subtitle = pack.subStrand
    ? t('understand.subStrand', { subStrand: pack.subStrand })
    : t('understand.fromCurriculum', { grade: pack.grade })
  ...
  return (
    <article className="flex flex-col rounded-lg border border-emerald-950/10 bg-white/50 p-5 shadow-[0_14px_36px_rgba(28,40,29,0.08)] backdrop-blur-sm dark:bg-white/5 dark:border-white/10 sm:p-6">
      <PanelHeader title={t('understand.title')} step={stepIndex('understand')} onBack={onBack} />
      ...
      <h2 className="mb-4 text-2xl font-black text-stone-950 dark:text-white">
        {t('understand.heading', { strand: pack.title })}
      </h2>
      ...
      <section className="overflow-hidden rounded-xl border border-emerald-950/10 bg-white/70 dark:border-white/10 dark:bg-white/5">
        <header className="flex items-center gap-3 border-b border-stone-200/80 bg-gradient-to-r from-emerald-50/80 to-transparent px-4 py-3 dark:border-white/10 dark:from-emerald-900/25">
          ...
          <div className="min-w-0">
            <strong className="block text-sm leading-tight text-stone-950 dark:text-white">
              {t('understand.aiName')}
            </strong>
            <span className="mt-0.5 block truncate text-xs text-stone-500 dark:text-stone-400">{subtitle}</span>
          </div>
        ...
        <footer className="border-t border-stone-200/80 px-4 py-2.5 text-xs leading-relaxed text-stone-500 dark:border-white/10 dark:text-stone-400">
          {t(isGenerated ? 'understand.footer' : 'understand.builtIn', { source: pack.source })}
        </footer>
      </section>

      <h3 className="mt-7 border-t border-stone-200 pt-4 text-sm font-extrabold uppercase dark:border-white/15 dark:text-white">
        {t('understand.outcomes')}
      </h3>
      <ul className="mt-3 grid gap-2">
        {pack.outcomes.map((item, index) => (
          <motion.li
            key={item}
            initial={{ opacity: 0, x: -8 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: index * 0.05, duration: 0.3 }}
            className="flex items-start gap-2 text-sm dark:text-stone-300"
          >
            <Check className="mt-0.5 shrink-0 rounded-full bg-emerald-700 p-0.5 text-white dark:bg-emerald-600" size={16} />
            {item}
          </motion.li>
        ))}
      </ul>

      <div className="mt-7 flex gap-3 rounded-lg border border-amber-200 bg-amber-50/50 p-4 backdrop-blur-sm dark:border-amber-900/40 dark:bg-amber-900/20 sm:mt-auto">
        <Sparkles className="shrink-0 text-amber-600 dark:text-amber-400" size={24} />
        <div>
          <strong className="text-amber-700 dark:text-amber-300">{t('understand.teacherNote')}</strong>
          <span className="mt-1 block text-sm leading-relaxed dark:text-amber-100/80">{pack.teacherNote}</span>
        </div>
      </div>
